...
# Version: 2.0
# Updated with production-ready Docker Compose configuration
...
get_server_ip() {
    local ip=$(curl -s ifconfig.me 2>/dev/null || curl -s icanhazip.com 2>/dev/null || curl -s ipecho.net/plain 2>/dev/null)
    echo "$ip"
}
